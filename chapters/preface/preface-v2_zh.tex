...
作为一本开源书籍，本书于2025年8月18日以“数据分布的深度表征学习”为题发布的第一版，在过去的六个月里被持续频繁的更新。
同时，结合我们在2025年秋季香港大学一门新课程中使用本书的经验，以及来自同事、助教和学生的反馈，
我们发现了书中许多值得进一步修订和扩充的内容。
...
最新发展的更深入的理论结果，以及越来越多针对真实世界数据的实际应用。据我们所知，这些内容
伤亡在其他地方得到妥善地整理和系统地解释。2.0 版正是对这一状况的及时补充。
...
\begin{itemize}
    \item 我们将第一版中的第3章拆分为两章，即现在的
        ...
        提供了理论依据。
    \item 在关于通过展开优化设计深度表征的更新后的第 \ref{ch:deep-networks} 章中，
    我们为白盒 Transformer 架构 CRATE 增加了一个因果版本的原理性推导，
    这对于处理我们在应用中常见的序列数据（如文本）非常重要。
    ...
    \item 扩充后的应用章节（第 \ref{ch:applications} 章）现在包含了
        更多针对真实世界数据（包括自然 2D 图像、自然 3D 物体形状、人体运动和自然语言）的分布学习和表征学习的详细实现，
        涵盖了几乎所有流行和实用的条件下的技术实现：有监督、弱监督、无监督和条件生成。 
    \item 关于开放方向的最后一章（第 \ref{ch:future} 章）也进行了
        大幅重写。我们试图对不同层次的智能给出一个更清晰的分类，以便我们能够
        （通过本书）阐明我们对智能已经做了什么、理解了什么，以及还有什么尚未实现和尚未理解。
        我们相信，与更高级形式的智能相关的开放性问题应该可以被很好地界定出来，从而可以
        通过科学和数学手段进行定性和定量的研究，而不是仅仅作为一个神秘的主题或纯粹的哲学话题来讨论。
\end{itemize}


\subsection*{2.0 版内容贡献者}
除了作者之外，在本书 {\em 2.0 版} 的准备过程中，更多学生和同事也开始加入这个开源项目，并为本书的不同部分贡献了更多宝贵的内容。
以下是部分贡献者及其具体贡献的名单（按姓名字母顺序排列）：
\begin{itemize}
