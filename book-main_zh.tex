% For HTML builds (tex4ht/make4ht), use the standard book class.
% EngC's internal font-switching machinery is incompatible with tex4ht's
% character-by-character hooks and breaks CJK processing.
\ifdefined\HCode
  \documentclass{book}
\else
  \documentclass{EngC}
...
% For tex4ht/make4ht HTML builds, map full-width punctuation to ASCII
\ifdefined\HCode
  % Map full-width comma (U+FF0C) to ASCII comma in tex4ht builds
  \catcode"FF0C=\active
  \def^^^^ff0c{,}
  % Map full-width colon (U+FF1A) to ASCII colon in tex4ht builds
  \catcode"FF1A=\active
  \def^^^^ff1a{:}
\fi
...
\usepackage{fancyvrb}
% soul is only needed for EngC's \spacedheader; skip under tex4ht (book class)
\makeatletter
\@ifclassloaded{EngC}{
    \usepackage{soul}
...
% EngC-specific: dot leaders in TOC, authorfooter, and custom maketitle.
% These use EngC internals (\AJW@toclmarg, \boldseriffont, \parttitlesize, etc.)
% and must be skipped when using the standard book class (HTML builds).
\makeatletter
\@ifclassloaded{EngC}{
  \newcommand{\AJW@tocleaders}{\leaders\hbox{$\m@th
    \mkern \@dotsep mu\hbox{.}\mkern \@dotsep mu$}\hfill}
  \newcommand{\tocleadersoff}{\let\AJW@tocleaders\hfill}
  \newcommand{\tocleaderson}{\let\AJW@tocleaders\AJW@tocleaders@saved}
  \let\AJW@tocleaders@saved\AJW@tocleaders
...
% Override \paragraph for Chinese: bold instead of italic, run-in (no newline after title)
\makeatletter
\@ifclassloaded{EngC}{
  \renewcommand\paragraph{\@startsection{paragraph}{4}{\z@}%
    {-13\p@ \@plus-7\p@ \@minus-3.5\p@}{-1em}{\Bheadsize\bfseries\AJW@raggedright}}
}{}
\makeatother
...
\@ifclassloaded{EngC}{
    ...
    \authorfooter{
        本书稿是为作者们将在各自机构讲授的深度学习与智能原理相关课程所编写的教材。 本预发布版本仅供个人免费浏览和下载使用，不得用于再分发、转售或制作衍生作品。
        ...
        Ma \\[\baselineskip]
        $^*$\,这些作者同等贡献。% These authors contributed equally.
    }
}{
...
\addtocontents{toc}{\protect\tocleadersoff}
...
\phantomsection\addcontentsline{toc}{chapter}{\numberline{}英文术语与中文翻译规范}
\subfile{chapters/translation_zh.tex}

\addtocontents{toc}{\protect\tocleaderson}
